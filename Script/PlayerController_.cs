...
public class PlayerController_ : MonoBehaviour
{
    ...
    public void PassCard(PlayerController_ newOwner,int cardIndex,string cardNewValue)
    {
        if(!GameSetup.GS.gameOver){
            ...
            playerCards[cardIndex].transform.localEulerAngles = cardRotation;
            if(GameSetup.GS.shuffled)
            {
                newOwner.PlayerHand.GetComponent<GridManager>().ArrangeCards();
            }
            ...
            if (!np.isBot){
                ...
                Locker.GetComponent<Collider>().enabled = true;
            }
